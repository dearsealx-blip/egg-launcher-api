...
// Embedded BOC codes (no filesystem access needed on Railway)
...
adminRouter.post('/redeploy-token', async (req, res) => {
    ...
    try {
        ...
        if (bal < toNano('0.7')) return res.status(400).json({ error: `Low balance: ${fromNano(bal)} TON`, address: eggWallet.toString({urlSafe:true,bounceable:false}) });
